...
public:
	...
	void addTriangle(const std::vector<Vec3f> TrianglePositon, const std::vector<Vec3f> NormalsPosition){
		size_t nb_vertex = vertexPositions().size();
		for (int i = 0; i < 3; i++)
			vertexPositions().push_back(TrianglePositon[i]);
		for (int i = 0; i < 3; i++)
			vertexNormals().push_back(NormalsPosition[i]);
		indexedTriangles().push_back(Triangle(nb_vertex,nb_vertex+1,nb_vertex+2));
	}
	...
	void recomputeNormals() {
		...
		N.resize(P.size(), Vec3f());			// compute 1 normal per vertex
		for (size_t i = 0; i < T.size(); i++) {
			Vec3f nt(triangleNormal(i));
			for (size_t j = 0; j < 3; j++)
				N[T[i][j]] += nt;				// average the normals over all adjacents triangle of vertex(T[i][j])
		}
	...
	inline void loadOFF(const std::string& filename) {
		// try {
			...
			// if (!in)
			// 	throw std::exception(("Error loading OFF file: " + filename).c_str());
		...
		// 	throw exception((string("Error Loading OFF file: ") + string(e.what())).c_str());
		// }
		recomputeNormals();
	}
